@echo off
@title The Sims4 Uncompyling tool
...
:rem ------------------------------------------------------
:rem ----------  1. Detect uncompyle6/decompile3 ---------- 
:rem ------------------------------------------------------
set pyminver=3.7.0
set dcminver=3.7.6
set ucminver=3.7.4
...
:rem ------------------------------------------------------
:rem ----------  2. Zip files discovering mods   ----------
:rem ------------------------------------------------------
...
:rem ------------------------------------------------------
:rem ----------  7. Run uncompilling scripts     ----------
:rem ------------------------------------------------------
rem start time
...
:rem ------------------------------------------------------
:rem ----------  8. Statistics - End of program  ----------
:rem ------------------------------------------------------
echo.
echo ==================================================
echo FIN DU PROCESS
...
:rem ---------------------------------------------------------------------------
:rem ----------            FUNCTIONS USED BY THE MAIN SCRIPT          ----------
:rem ---------------------------------------------------------------------------
:VersionCheck
    set "program=%~1"
    set "cur_ver=%~2"
...
:InstallRequirements
    ...
    CALL :RunRequirementDetectiion
    if "%PREREQUIRED_NOT_SATISFIED%"=="1" goto :EndProgramFailure

exit /b


:RunRequirementDetectiion 

    rem checking is the right version of pytho is detected on the machine
    for /f "tokens=2" %%F in ('py --version 2^>nul') do (
        set "py3=%%F"
    )
    if not defined py3 (
        echo CRITICAL ERROR DETECTED : The python ^(version 3.7.x^) is not detected.
        echo Please install it. 
        echo Note : This version is important because the Sims4 has been published with code 3.7.x. 
        echo 
        goto :EndProgramFailure
    )

    rem Checking if the decompyle 3 scrIpt is found the the machine
    ...
    rem Checking if the uncompyle 3 scrIpt is found the the machine
    ...
    rem: if the program goes well, all the prerequires are satisfied 
    SET PREREQUIRED_NOT_SATISFIED=0

    rem version check now in a function
